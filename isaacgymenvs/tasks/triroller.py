...
class TrirollerDimensions(enum.Enum):
	"""
...
class Triroller(VecTask):
	...
	_trirollers_assets_dir = os.path.join(project_dir, "../", "assets", "triroller")
	# robot urdf (path relative to `_trirollers_assets_dir`)
	_robot_urdf_file = "robot_properties_fingers/urdf/pro/triroller.urdf"
	# stage urdf (path relative to `_trirollers_assets_dir`)
	...
	# object urdf (path relative to `_trirollers_assets_dir`)
	...
	_robot_limits: dict = {
		...
		"joint_torque": SimpleNamespace(
			...
			default=(np.array(_min_torque_Nm, dtype=np.float32)+np.array(_max_torque_Nm, dtype=np.float32))/2,
		),
	...
	def __init__(self, cfg, sim_device, graphics_device_id, headless):
		...
		fingertips_frames = ["l1_right", "l2_right", "l3_right"]
		...
		for dof_name in ['front', 'left', 'right']:
			for robot_id in ['r1', 'r2', 'r3']:
				robot_dof_names.append(f'{robot_id}_{dof_name}')
	...
	def __define_robot_asset(self):
		...
		trirollers_asset = self.gym.load_asset(self.sim, self._trirollers_assets_dir,
		...
		if self.cfg["env"]["enable_ft_sensors"] or self.cfg["env"]["asymmetric_obs"]:
			...
			for fingertip_handle in self._fingertips_handles.values():
				self.gym.create_asset_force_sensor(trirollers_asset, fingertip_handle, sensor_pose)
		...
		for dof_name in self._robot_dof_indices.keys():
			self._robot_dof_indices[dof_name] = self.gym.find_asset_dof_index(trirollers_asset, dof_name)
			# check valid handle
	...
	def __define_table_asset(self):
		...
		# load stage asset
		table_asset = self.gym.load_asset(self.sim, self._trirollers_assets_dir,
	...
	def __define_boundary_asset(self):
		...
		# load stage asset
		boundary_asset = self.gym.load_asset(self.sim, self._trirollers_assets_dir,
	...
	def __define_object_asset(self):
		...
		# load object asset
		object_asset = self.gym.load_asset(self.sim, self._trirollers_assets_dir,
	...
	def __define_goal_object_asset(self):
		...
		object_asset_options.flip_visual_attachments = True
		# load object asset
		goal_object_asset = self.gym.load_asset(self.sim, self._trirollers_assets_dir,
												 self._object_urdf_file, object_asset_options)
		# return the asset
...
def main(cfg):
	env = Triroller(cfg=omegaconf_to_dict(cfg.task), sim_device='cuda:0', graphics_device_id=0, headless=False)
	for _ in range(1000):
		env.step(torch.rand((env.num_envs, env.action_dim), dtype=torch.float, device='cuda:0')*2-1)
