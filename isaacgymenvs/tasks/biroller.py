...
class BirollerDimensions(enum.Enum):
  """
  Dimensions of the bi-finger robot.

  Note: While it may not seem necessary for bi-finger robot since it is fixed base, for floating
  ...
  NumFingers = 2
  # for two fingers
  JointPositionDim = 6 + 1
  JointVelocityDim = 6 + 1
  JointTorqueDim = 6 + 1 # use wrist
  ActionDim = 3
...
class Biroller(VecTask):
  ...
  _robot_limits: dict = {
    "action": SimpleNamespace(
      # matches those on the real robot pitch, roll, moving along, wrist
      low=np.array([-10, -30, -10, -10], dtype=np.float32),
      high=np.array([10, 30, 10, 10], dtype=np.float32),
      default=np.array(
        [0.0, 0., 0.0, 0.0], dtype=np.float32),
    ),
    "joint_vel": SimpleNamespace(
      # pitch_l, pitch_r, roll_l, roll_r, wrist
      low=np.array([-10, -10, -20, -20, -10], dtype=np.float32),
      high=np.array([10, 10, 20, 20, 10], dtype=np.float32),
      default=np.array(
    ...
    "joint_velocity": SimpleNamespace(
      low=np.full(_dims.JointVelocityDim.value, - \
                  _max_velocity_radps, dtype=np.float32),
      high=np.full(_dims.JointVelocityDim.value,
                   _max_velocity_radps, dtype=np.float32),
      default=np.zeros(_dims.JointVelocityDim.value, dtype=np.float32),
    ),
    "joint_torque": SimpleNamespace(
      low=np.array(_min_torque_Nm, dtype=np.float32),
      high=np.array(_max_torque_Nm, dtype=np.float32),
      default=(np.array(_min_torque_Nm, dtype=np.float32) + \
               np.array(_max_torque_Nm, dtype=np.float32))/2,
    ),
  ...
  def __init__(self, cfg, sim_device, graphics_device_id, headless):
    ...
    if self.cfg["env"]["enable_ft_sensors"] or self.cfg["env"]["asymmetric_obs"]:
      ...
      self._ft_sensors_values = gymtorch.wrap_tensor(
        sensor_tensor).view(self.num_envs, num_ft_dims)
    ...
    # root actors
    self._actors_root_state = gymtorch.wrap_tensor(
    ...
    while curr_history_length < self._state_history_len:
      ...
      self._object_state_history.append(
        self._actors_root_state[object_indices])
  ...
  def _create_scene_assets(self):
    ...
    self.object_texture = self.gym.create_texture_from_file(
      self.sim, f'{self._birollers_assets_dir}/objects/rounded_cube.png')
  ...
  def _create_envs(self, num_envs, spacing, num_per_row):
    ...
    for k, dof_index in enumerate(self._robot_dof_indices.values()):
      ...
      # set dof limits
      robot_dof_props['effort'][dof_index] = self._max_torque_Nm[dof_index]
    ...
    env_lower_bound = gymapi.Vec3(-self.cfg["env"]
                                  ["envSpacing"], -self.cfg["env"]["envSpacing"], 0.0)
    env_upper_bound = gymapi.Vec3(
      self.cfg["env"]["envSpacing"], self.cfg["env"]["envSpacing"], self.cfg["env"]["envSpacing"])
    ...
    for env_index in range(self.num_envs):
      ...
      self.gym.set_rigid_body_texture(
        env_ptr, object_handle, 0, gymapi.MESH_VISUAL, self.object_texture)
      ...
      self.gym.set_rigid_body_color(
        env_ptr, boundary_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, stage_color)
  ...
  def __configure_mdp_spaces(self):
    ...
    if self.cfg["env"]["normalize_action"]:
      obs_action_scale = SimpleNamespace(
        low=torch.full((self.action_dim,), -1,
                       dtype=torch.float, device=self.device),
        high=torch.full((self.action_dim,), 1,
                        dtype=torch.float, device=self.device)
      )
    else:
    ...
    if self.cfg["env"]["asymmetric_obs"] \
      ...
      msg = f"States scaling dimensions mismatch. " \
        ...
        f"\tExpected: {state_dim}."  # define by state_spec (generated)
      raise AssertionError(msg)
    # actions
  ...
  def compute_reward(self, actions):
    ...
    self.extras.update({f"env/rewards/{k}": v.mean()
  ...
  def compute_observations(self):
    ...
    if self.cfg["env"]["enable_ft_sensors"] or self.cfg["env"]["asymmetric_obs"]:
      ...
      tip_wrenches = torch.zeros(self.num_envs, self._dims.NumFingers.value *
                                 self._dims.WrenchDim.value, dtype=torch.float32, device=self.device)
    ...
    self._fingertips_frames_state_history.appendleft(
      self._rigid_body_state[:, fingertip_handles_indices])
    self._object_state_history.appendleft(
      self._actors_root_state[object_indices])
  ...
  def _sample_robot_state(self, instances: torch.Tensor, distribution: str = 'default',
    ...
    if distribution == "none":
      ...
      dof_state_dim = self._dims.JointPositionDim.value + \
        self._dims.JointVelocityDim.value
      ...
      self._dof_position[instances, pl_id] += dof_pos_stddev * \
        dof_state_noise[:, pl_id]
      self._dof_position[instances, pr_id] += dof_pos_stddev * \
        dof_state_noise[:, pr_id]
      # DOF velocity
      # start_offset = end_offset
      # end_offset += self._dims.JointVelocityDim.value
      # self._dof_velocity[instances] += dof_vel_stddev * \
      #   dof_state_noise[:, start_offset:end_offset]
    else:
  ...
  def _sample_object_goal_poses(self, instances: torch.Tensor, difficulty: int):
    ...
    Possible levels are:
      ...
      - 5: Random goal orientation only
    """
    # number of samples to generate
    num_samples = instances.size()[0]
    ...
    if difficulty == -1:
      ...
      pos_x, pos_y = random_xy(
        num_samples, self._object_dims.max_com_distance_to_center, self.device)
      pos_z = random_z(num_samples, self._object_dims.min_height,
                       self._object_dims.max_height, self.device)
      orientation = default_orientation(num_samples, self.device)
    elif difficulty == 4:
      ...
      pos_z = random_z(num_samples, self._object_dims.radius_3d,
                       max_height, self.device)
    elif difficulty == 5:
      max_goal_radius = self._object_dims.max_com_distance_to_center
      max_height = self._object_dims.max_height
      orientation = random_orientation(num_samples, self.device)
    ...
    # root actor buffer
    self._actors_root_state[goal_object_indices,
                            0:7] = self._object_goal_poses_buf[instances]
  ...
  def pre_physics_step(self, actions):
    ...
    if self.cfg["env"]["command_mode"] == 'torque':
      ...
      pl_id = self._robot_dof_indices['pitch_l']
      pr_id = self._robot_dof_indices['pitch_r']
      desired_dof_position[..., pl_id] += pitch_vel * self.cfg["sim"]["dt"]
      desired_dof_position[..., pr_id] += pitch_vel * self.cfg["sim"]["dt"]
      # get roll angle
      rl_id = self._robot_dof_indices['roll_l']
      rr_id = self._robot_dof_indices['roll_r']
      desired_dof_position[...,
                           rl_id] += (roll_vel+move_vel) * self.cfg["sim"]["dt"]
      desired_dof_position[...,
                           rr_id] += (roll_vel-move_vel) * self.cfg["sim"]["dt"]
      # get wrist angle
    ...
    if self.cfg["env"]["apply_safety_damping"]:
      ...
      applied_torque -= self._robot_dof_gains["safety_damping"] * \
        self._dof_velocity
      # clamp input
      applied_torque = saturate(
        applied_torque,
  ...
  def _check_termination(self):
    ...
    object_goal_position_dist = torch.norm(
      self._object_goal_poses_buf[:, 0:3] -
      self._object_state_history[0][:, 0:3],
      p=2, dim=-1
    )
    ...
    elif self.cfg["env"]['task_difficulty'] == 5:
  ...
  def __define_robot_asset(self):
    ...
    trirollers_asset = self.gym.load_asset(self.sim, self._birollers_assets_dir,
    ...
    self.gym.set_asset_rigid_shape_properties(
      trirollers_asset, trifinger_props)
    # extract the frame handles
    for frame_name in self._fingertips_handles.keys():
      self._fingertips_handles[frame_name] = self.gym.find_asset_rigid_body_index(trirollers_asset,
                                                                                  frame_name)
      # check valid handle
    ...
    if self.cfg["env"]["enable_ft_sensors"] or self.cfg["env"]["asymmetric_obs"]:
      ...
      for fingertip_handle in self._fingertips_handles.values():
        self.gym.create_asset_force_sensor(
          trirollers_asset, fingertip_handle, sensor_pose)
  ...
  def __define_table_asset(self):
    ...
    # load stage asset
    table_asset = self.gym.load_asset(self.sim, self._birollers_assets_dir,
  ...
  def __define_boundary_asset(self):
    ...
    # load stage asset
    boundary_asset = self.gym.load_asset(self.sim, self._birollers_assets_dir,
  ...
  def __define_object_asset(self):
    ...
    # load object asset
    object_asset = self.gym.load_asset(self.sim, self._birollers_assets_dir,
  ...
  def __define_goal_object_asset(self):
    ...
    object_asset_options.flip_visual_attachments = True
    # load object asset
    goal_object_asset = self.gym.load_asset(self.sim, self._birollers_assets_dir,
                                            self._object_urdf_file, object_asset_options)
    # return the asset
    return goal_object_asset

  def ezpolicy(obs, mode='default'):
    if mode == 'default':
      # ===parse observation===
      obj_pos = obs[12:12+7]
      goal_pos = obs[19:19+7]
      obs = {
        'obj_state': goal_pos[3:7],
        'goal_state': obj_pos[3:7],
      }
      goal_orn = R.from_quat(np.concatenate([obs['goal_state'][...,1:], obs['goal_state'][...,[0]]], axis=-1))
      obj_orn = R.from_quat(
        np.concatenate([obs['obj_state'][...,1:], obs['obj_state'][...,[0]]], axis=-1))
...
def compute_trifinger_reward(
  ...
  if use_keypoints:
    ...
    object_keypoints -= object_center
    goal_keypoints = gen_keypoints(object_goal_poses_buf[:, 0:7])
    goal_center = goal_keypoints.mean(dim=-1, keepdim=True)
    goal_keypoints -= goal_center
    ...
    object_dist_reward = object_dist_weight * dt * \
      lgsk_kernel(object_dist, scale=50., eps=2.)
    ...
    object_rot_reward = object_rot_weight * \
      dt / (3. * torch.abs(angles) + 0.01)
...
def random_orientation_within_angle(num: int, device: str, base: torch.Tensor, max_angle: float):
  ...
  quat[:, 2] = (rand[:, 1]*2.-1.) * n
  quat[:, 0] = (torch.sqrt(1-quat[:, 2]**2.) *
                torch.cos(2*np.pi*rand[:, 2])) * n
  quat[:, 1] = (torch.sqrt(1-quat[:, 2]**2.) *
                torch.sin(2*np.pi*rand[:, 2])) * n
